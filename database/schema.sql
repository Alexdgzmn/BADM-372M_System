...
CREATE TABLE notifications (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    notification_type VARCHAR(50) CHECK (notification_type IN ('mission_reminder', 'challenge_invite', 'friend_request', 'achievement_earned', 'streak_reminder', 'post_interaction')),
    title VARCHAR(200) NOT NULL,
    message TEXT NOT NULL,
    is_read BOOLEAN DEFAULT FALSE,
    action_url TEXT,
...
CREATE INDEX idx_user_skills_level ON user_skills(level DESC);
...
CREATE INDEX idx_missions_completed ON missions(is_completed, created_at);
...
-- Rankings indexes
CREATE INDEX idx_user_rankings_type_rank ON user_rankings(ranking_type, rank_position);
...
-- Sample notification types can be added here as needed